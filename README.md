...
- 长音频会先分片再逐段转写，降低 Apple Speech 漏转/截断概率
- 支持对最新 `.m4a` 录音重新转写
- 输出原始转写 `.txt`
- 输出阅读稿 `.md`
- 同时保留录音 `.m4a`
...
如果录音文件存在但转写不完整，可以点击 `转写最新录音`，App 会对最新 `.m4a` 重新分片转写并覆盖对应 `.txt/.md`。
...
- 转写用 Apple Speech，支持时优先使用本地识别；App 会自动分片转写长音频，但准确率仍取决于系统服务。
